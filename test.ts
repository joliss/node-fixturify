...
test('error conditions', function (t: any) {
  ...
  test('writeSync guards against misuse that could cause data loss', function (t: any) {
    ...
    t.throws(() => fixturify.writeSync('testdir.tmp', { '': 'contents' }), /non-empty string/);
    t.throws(() => fixturify.writeSync('testdir.tmp', { '.': { } }), /must not be "\." or "\.\."/);
    t.throws(() => fixturify.writeSync('testdir.tmp', { '..': { } }), /must not be "\." or "\.\."/);
    t.throws(() => fixturify.writeSync('testdir.tmp', { 'foo/bar': { } }), /must not contain "\/" or "\\"/);
    t.throws(() => fixturify.writeSync('testdir.tmp', { 'foo\\bar': { } }), /must not contain "\/" or "\\"/);
    t.end();
  });
